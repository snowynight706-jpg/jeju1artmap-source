...
지도 조작은 제스처 중 합성 변형을 사용하고, 실제 이동이 시작되면 라벨 페인트를 잠시 중단한 뒤 종료 후 실제 확대율과 라벨 배치를 한 번 확정한다. 라벨은 투명도 전환으로 복구하고 상태표시는 장소명 바깥 상단 줄에 둔다. 공개 모바일 렌더 계층은 현재 화면의 넉넉한 여유 범위 안의 마커·통합 라벨만 받는다. 장소 목록과 데이터 계산은 전체 공개 요소를 계속 사용하며, 선택 장소의 상세·행사·후기 UI는 별도 지연 로딩 묶음으로 제공한다. 베이스맵은 확대 중 소스를 바꾸지 않고 고화질 화면용 이미지 한 장을 유지한다. 공개 패널 이동은 History API 상태와 지도 선택 상태를 함께 관리한다.
