...
화면용 라벨 후보는 인쇄 미리보기를 제외하고 확정된 지도 이동·배율에서 계산한 현재 화면 경계 안의 장소로 먼저 제한한 뒤 축척별 전체 라벨 예산과 밀집 통합을 적용한다. 이동·확대 중에는 이전 라벨을 숨긴다. 조작 종료 커밋에서는 실제 지도 폭과 하단 축척 상태를 함께 확정하고, 다음 프레임에 라벨 구성·통합 라벨·연결선·EVENT/NEW 상태를 함께 갱신해 0.12초 투명도 전환으로 복구한다. 관리자 편집 지도는 기존 0.28초 전환을 유지한다. 공개 하단의 새로고침은 네트워크나 베이스맵을 다시 읽지 않고 이 화면 경계와 라벨·마커 렌더 계층만 재계산한다.
