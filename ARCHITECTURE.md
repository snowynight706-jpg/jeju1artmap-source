...
- `app/site-identity.ts`: 사이트 표시 이름의 기본값·길이 제한·공백 정규화와 공개/관리자 공용 설정 계약
...
사이트 표시 이름의 정적 최초값과 PWA 설치 이름은 `제주 원도심 맵`이다. 런타임 표시 이름은 `site_identity_settings`의 단일 행 독립 설정으로 저장한다. 공개 레이아웃 최초 응답의 기존 D1 배치 읽기에 함께 포함해 추가 네트워크 요청 없이 공개·관리자 헤더와 브라우저 메타데이터를 갱신하고, 장소 공유 제목도 같은 값을 사용한다. 저장은 설정 revision을 조건으로 갱신해 여러 관리자 화면의 동시 변경을 감지하며 공개 지도 문서와 이력에는 영향을 주지 않는다.

화면용 라벨 후보는 인쇄 미리보기를 제외하고 확정된 지도 이동·배율에서 계산한 현재 화면 경계 안의 장소로 먼저 제한한 뒤 축척별 전체 라벨 예산과 밀집 통합을 적용한다. 이동·확대 중에는 이전 라벨을 숨긴다. 조작 종료 시 합성 변형은 React 확대율이 확정된 레이아웃 이펙트까지 유지하고, 같은 페인트 전 단계에서 실제 지도 폭과 하단 축척 상태를 확정한 뒤 제거한다. 다음 프레임에는 라벨 구성·통합 라벨·연결선·EVENT/NEW 상태를 함께 갱신해 0.12초 투명도 전환으로 복구한다. 관리자 편집 지도는 기존 0.28초 전환을 유지한다. 공개 하단의 새로고침은 네트워크나 베이스맵을 다시 읽지 않고 이 화면 경계와 라벨·마커 렌더 계층만 재계산한다.
