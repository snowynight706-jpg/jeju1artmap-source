...
사이트 표시 이름은 `site_identity_settings`의 단일 행 독립 설정으로 저장한다. 공개 레이아웃 최초 응답의 기존 D1 배치 읽기에 함께 포함해 추가 네트워크 요청 없이 공개·관리자 헤더와 브라우저 메타데이터를 갱신하고, 장소 공유 제목도 같은 값을 사용한다. 저장은 설정 revision을 조건으로 갱신해 여러 관리자 화면의 동시 변경을 감지하며 공개 지도 문서와 이력에는 영향을 주지 않는다.
