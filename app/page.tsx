...
const initialElements: MapElement[] = [
  { id: "dummy-1", name: "아라리오뮤지엄", category: "landmark", x: 18, y: 24, anchorX: 18, anchorY: 24, size: 7, z: 1, labelVisible: true, labelPosition: "bottom", status: "unchecked", locked: false },
  { id: "dummy-2", name: "관덕정", category: "landmark", x: 38, y: 59, anchorX: 38, anchorY: 59, size: 6.2, z: 2, labelVisible: true, labelPosition: "bottom", status: "review", locked: false },
  { id: "dummy-3", name: "동문시장", category: "landmark", x: 66, y: 55, anchorX: 66, anchorY: 55, size: 6.8, z: 3, labelVisible: true, labelPosition: "bottom", status: "review", locked: false },
  { id: "dummy-4", name: "문화시설 A", category: "culture", x: 52, y: 42, anchorX: 52, anchorY: 42, size: 3, z: 4, labelVisible: false, labelPosition: "right", status: "unchecked", locked: false },
  { id: "dummy-5", name: "카페 A", category: "cafe", x: 56, y: 46, anchorX: 56, anchorY: 46, size: 1.7, z: 5, labelVisible: false, labelPosition: "right", status: "unchecked", locked: false },
  { id: "dummy-6", name: "음식점 A", category: "food", x: 60, y: 48, anchorX: 60, anchorY: 48, size: 1.7, z: 6, labelVisible: false, labelPosition: "right", status: "unchecked", locked: false },
  { id: "dummy-7", name: "주차장 A", category: "parking", x: 43, y: 67, anchorX: 43, anchorY: 67, size: 1.6, z: 7, labelVisible: false, labelPosition: "right", status: "unchecked", locked: false },
];
...
function labelClass(position: LabelPosition) {
...
export default function Home() {
  ...
  const [viewMode, setViewMode] = useState<"all" | "landmarks" | "markers">("all");
  ...
  const visibleElements = useMemo(() => {
    return [...elements]
      ...
      .filter((element) => viewMode === "all" || (viewMode === "landmarks" ? element.category === "landmark" : element.category !== "landmark"))
      .sort((a, b) => a.z - b.z);
    ...
    <main className="app-shell">
      ...
      <section className={`workspace ${leftOpen ? "" : "left-closed"} ${rightOpen ? "" : "right-closed"}`}>
        <aside className="panel asset-panel" aria-label="자산 목록">
          ...
          <div className="asset-list-header"><span>더미 자산</span><small>클릭하여 중앙에 추가</small></div>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          zIndex: element.z, color: meta.color,
                        }}
                      ...
                      >
                        ...
                        {element.labelVisible && <div className={labelClass(element.labelPosition)}>{element.name}</div>}
        ...
        <aside className="panel properties-panel" aria-label="속성 편집">
          ...
          {!selected ? (
            ...
            <div className="property-form">
              ...
              <section>
                <div className="section-title"><strong>실제 위치 앵커</strong><span>1차 임시값</span></div>
                <div className="field-row disabled-fields">
                  <label>X<input value={selected.anchorX.toFixed(2)} disabled /></label>
                  <label>Y<input value={selected.anchorY.toFixed(2)} disabled /></label>
                </div>
                <p className="field-help">장소 DB가 확인되면 실제 좌표와 시각적 배치 좌표를 분리합니다.</p>
              </section>
              <section>
