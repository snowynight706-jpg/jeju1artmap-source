...
test("mobile overview budgets stay compact while the initial detail scale keeps every nearby marker", () => {
  ...
  assert.equal(mobileMarkerBudgetForScale(1.15, 0.5, 200, "low"), 200);
  assert.equal(mobileMarkerBudgetForScale(1.15, 0.5, 200, "standard"), 200);
...
test("landmarks and the selected place stay full while optional marker slots follow viewport distance", () => {
  const candidates = [
  ...
  assert.deepEqual(new Set(ids), new Set(["landmark", "selected", "near"]));
...
test("mobile PWA uses a light root fallback and lightweight category-colored omitted markers", () => {
  ...
  assert.match(cssSource, /\.mobile-marker-placeholder::after \{[^}]*width: 8px; height: 8px; border: \.75px solid #666b6a/);
  ...
  assert.match(pageSource, /mobileMarkerBudgetForScale\(\s*zoom,\s*fitZoom,\s*mobileMapCandidateElements\.length,\s*mobileRenderBudget\.tier,?\s*\)/);
  assert.doesNotMatch(pageSource, /mobileMarkerBudgetForScale\([\s\S]{0,180}startupInitialViewTarget\?\.zoom/);
